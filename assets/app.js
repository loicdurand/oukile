...
init(() => document.addEventListener("turbo:load", init));

function init(callback) {
    onReady(".action_edit").then(() => {
        [...document.getElementsByClassName("action_edit")].forEach(
            (lien_editable) => {
...
function onceEdited(e) {
    ...
    setTimeout(async () => {
        ...
        if (oldText === newText) return deletebtn.classList.add("fr-hidden");
        const [, method, entity, id, attr] = target.id.split("_");
        axios[method](
            `/oukile/api/${entity}/${id}`,
            {
                [attr]: newText,
            },
            AXIOS_HEADERS,
